...
function initCarousel() {
  ...
  slides.forEach((_, i) => {
    ...
    if (i === slides.length - 1) { dot.classList.add('progress-dot-dashboard'); }
  ...
  const elems = slides.slice(0, -1).map((slide, i) => {
    ...
    // Add notification text to the "Where are you from?" slide
    if (slide.headline === "Where are you from?") {
      ...
      const navControls = el.querySelector('.nav-controls');
      navControls.parentNode.insertBefore(notificationText, navControls);
    }
...
function navigateTo(index) {
  ...
  if (index < 0 || index >= slides.length - 1 || index === current) { return; }
...
function updateProgress(currentIndex) {
  ...
  dots.forEach((dot, i) => {
    ...
    dot.style.cursor = (i !== currentIndex) ? 'pointer' : 'default';
  });
}
...
function renderSlideContent(idx) {
  ...
  if (title.startsWith("Does Money Buy Happiness") && !document.querySelector("#scatter-slide svg")) {
    drawScatter("scatter-slide");
  }
  if (title === "World Map" && !document.querySelector("#map-container svg")) {
